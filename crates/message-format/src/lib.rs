...
mod tests {
    ...
    fn localized(tag: &str, source: &str) -> LocalizedCatalog {
        LocalizedCatalog::new(locale(tag), MessageCatalog::compile_str(source).unwrap_or_else(|e| panic!("compile {tag}: {e:?}")))
    }
    ...
    fn bundle_lookup_falls_back_to_parent_locale_catalog() {
        let bundle = CatalogBundle::new(
            [localized("fr", "Salut { $name }"), localized("en", "Hello { $name }")],
            &locale("fr-CA"),
        )
        .expect("bundle");
    ...
    fn bundle_formatter_resolves_named_args_against_active_catalog() {
        let bundle = CatalogBundle::new(
            [localized("en", "Hello { $name }"), localized("fr", "Salut { $given } { $name }")],
            &locale("en-AU"),
        )
        .expect("bundle");
    ...
    fn bundle_lookup_uses_cldr_parent_locale() {
        ...
        let bundle = CatalogBundle::new(
            [localized("pt-PT", "Olá { $name }"), localized("pt", "Oi { $name }")],
            &locale("pt-MZ"),
        )
        .expect("bundle");
